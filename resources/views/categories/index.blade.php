...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('categories.store') }}" method="post" class="flex flex-col gap-2 md:flex-row md:items-end bg-white p-4 rounded-2xl shadow mb-4">
                @csrf
                <input name="name" class="flex-1 w-full border rounded p-2" placeholder="Nama kategori" required>
                <input name="code_category" class="flex-1 w-full border rounded p-2" placeholder="Kode Kategori" required>
                <button class="w-full md:w-auto px-4 py-2 rounded bg-slate-800 text-white">Simpan</button>
            </form>
            <div class="bg-white rounded-2xl shadow">
                <div class="p-4 overflow-x-auto">
                    <table id="tabelKategori" class="min-w-[40rem] w-full text-sm">
                        ...
                        <x-modal name="edit-category-{{ $c->id }}" :show="false" maxWidth="md">
                            ...
                            <form action="{{ route('categories.update', $c) }}" method="post" class="p-6 flex flex-col gap-4">
                                ...
                                <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                                    <button type="button" @click="$dispatch('close-modal', 'edit-category-{{ $c->id }}')" class="w-full sm:w-auto px-4 py-2 rounded border">Batal</button>
